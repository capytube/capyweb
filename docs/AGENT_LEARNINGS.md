...
- 2026-09-26: `apigateway` WAS granted after all — a fifth policy `capyapp-deploy-apigw` was attached to `capyapp-deployers` at 2026-09-24T09:59:38Z, after the 09-24 probe found it absent. It covers `apigateway:*` on `/apis` and `/apis/*` in ap-southeast-1 and us-east-1, and Denies non-GET on five pre-existing APIs plus `/account` and `/vpclinks`. **Re-probe permissions before concluding something is unavailable — this policy set is being actively edited.**
- 2026-09-26: **Cognito has no grant anywhere** across all five attached policies, and `cognito-idp:ListUserPools` is denied live. `capyweb-w26` (user pool + native JWT authorizer) cannot be built until `cognito-idp:*` on `capyapp-*` is added.
- 2026-09-26: git remote is HTTPS and there is no `gh` CLI and no stored credential, so `git push` fails with "could not read Username". SSH works: `ssh -T git@github.com` authenticates as `thanakijwanavit`. Push by URL: `git push git@github.com:capytube/capyweb.git <branch>:refs/heads/<branch>`.

## Guards (2026-09-26, capyweb-1nh)
- `scripts/guard.sh` is the cost/security/build gate. There is NO CI (Actions banned), so it runs as a git hook: `scripts/install-hooks.sh` installs pre-commit (guards) and pre-push (guards + backend tests).
- `scripts/guard-selftest.sh` proves every rule actually fires, by copying the repo to a scratch dir, introducing one violation at a time and asserting the failure. **Run it after changing guard.sh** — two rules were silently weak until it was written (the log-retention count was a broken `bc` pipeline, and the playback check grepped a variable name, so renaming the constant silenced it).
